...
class HostConfigCollector:
    ...
    def _is_config_complete(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for field in system_fields:
                if field not in config["system"] or config["system"][field] == "N/A":
                    log.debug(f"配置缺少或无效的system字段: {field}")
                    return False

            # 检查python字段是否有效
            if (
                "python_version" not in config["python"]
                or config["python"]["python_version"] == "N/A"
            ):
                log.debug(f"配置缺少或无效的python_version")
                return False

            # 检查libraries是否为空或全是"Not installed"
            if not config["libraries"] or all(
                v == "Not installed" for v in config["libraries"].values()
            ):
                log.debug(f"配置缺少有效的库信息")
                return False
    ...
    def _generate_change_summary(self, changes: Dict[str, Any]) -> str:
        ...
        lib_changes = [
            ...
            if k.startswith("lib_") and not k.startswith("lib_status_")
        ]
        ...
        for key, change_info in changes.items():
            if key == "initial":
                ...
                summary_parts.append(
                    f"{key}: {change_info['old']} -> {change_info['new']}"
                )
            else:
    ...
    def get_python_info(self):
        ...
        try:
            ...
            python_info["pip_version"] = "Unknown"
        # 获取虚拟环境信息
        python_info["virtual_env"] = os.environ.get(
            "VIRTUAL_ENV", "Not in virtual environment"
        )
        ...
        for config_file in config_files:
            ...
            if file_path.exists():
                ...
                try:
                    ...
                    project_info["config_files"][f"{config_file}_mtime"] = (
                        mtime.isoformat()
                    )
                except:
                    pass
    ...
    @timethis
    def save_config_with_change_detection(
    ...
    def show_config_summary(self):
        ...
        print(
            f"   系统: {config['system']['system'].get('distro', config['system']['system']['platform'])}"
        )
        ...
        print(
            f"   文件大小: {self.local_config_file.stat().st_size if self.local_config_file.exists() else 0} bytes"
        )
...
class JoplinConfigManager:
            ...
            if not update_file.exists():
                ...
                try:
                    ...
                    # 或者如果内容有变化（比较最新记录的时间戳）
                        ...
                        latest_existing = (
                            existing_records[0].get("timestamp")
                            if existing_records
                            else None
                        )
    ...
    def parse_config_from_markdown_table(
        ...
        try:
            ...
            for device_name in device_names:
                device_id = findvaluebykeyinsection(
                    "happyjpinifromcloud", "device", device_name
                )
                if not device_id or device_id == "None":
                    ...
                    device_id = (
                        f"joplin_{hashlib.md5(device_name.encode()).hexdigest()[:8]}"
                    )
                    log.warning(
                        f"无法找到设备 {device_name} 的ID，使用哈希ID: {device_id}"
                    )
                ...
                if current_section == "system":
                    ...
                    for j, device_name in enumerate(device_names):
                        if j + 1 < len(cells):
                            ...
                            # 根据配置项填充数据
                            if config_item == "操作系统":
                                configs[device_id]["system"]["system"]["platform"] = (
                                    value
                                )
                            elif config_item == "内核版本":
                                configs[device_id]["system"]["system"]["release"] = (
                                    value
                                )
                            elif config_item == "架构":
                                ...
                                if "(" in value:
                                    ...
                                    configs[device_id]["system"]["system"][
                                        "machine"
                                    ] = machine
                                else:
                                    configs[device_id]["system"]["system"][
                    ...
                    for j, device_name in enumerate(device_names):
                        if j + 1 < len(cells):
                            ...
                            if config_item == "项目路径":
                                ...
                                configs[device_id]["project"]["config_files"][
                                    "requirements.txt"
                                ] = {
            ...
            for device_id, config in configs.items():
                ...
                log.info(
                    f"设备 {device_name}: Python={python_version}, 库数量={lib_count}, 收集时间={collection_time}, 更新记录数={update_count}"
                )
    ...
    def _merge_configs(
        ...
        if "system" in local_config:
            if (
                "device_id" not in merged["system"]
                or merged["system"]["device_id"] == "N/A"
            ):
                merged["system"]["device_id"] = local_config["system"].get(
                    "device_id", "N/A"
                )

            if (
                "device_name" not in merged["system"]
                or merged["system"]["device_name"] == "N/A"
            ):
                merged["system"]["device_name"] = local_config["system"].get(
                    "device_name", "N/A"
                )

            if (
                "host_user" not in merged["system"]
                or merged["system"]["host_user"] == "N/A"
            ):
                merged["system"]["host_user"] = local_config["system"].get(
                    "host_user", "N/A"
                )

            if (
                "timestamp" not in merged["system"]
                or merged["system"]["timestamp"] == "N/A"
            ):
                merged["system"]["timestamp"] = local_config["system"].get(
                    "timestamp", "N/A"
                )
            ...
            if "system" in local_config["system"]:
                for key in [
                    ...
                    if (
                        key not in merged["system"]["system"]
                        or merged["system"]["system"][key] == "N/A"
                    ):
                        if key in local_config["system"]["system"]:
                            merged["system"]["system"][key] = local_config["system"][
                                "system"
                            ][key]
        ...
        if "libraries" in local_config and local_config["libraries"]:
            if not merged["libraries"] or all(
                v == "Not installed" for v in merged["libraries"].values()
            ):
                ...
                for lib, version in local_config["libraries"].items():
                    if (
                        lib not in merged["libraries"]
                        or merged["libraries"][lib] == "Not installed"
                    ):
        ...
        if "project" in local_config:
            if (
                "project_path" not in merged["project"]
                or merged["project"]["project_path"] == "N/A"
            ):
                merged["project"]["project_path"] = local_config["project"].get(
                    "project_path", "N/A"
                )

            if (
                "config_files" not in merged["project"]
                or not merged["project"]["config_files"]
            ):
                merged["project"]["config_files"] = local_config["project"].get(
                    "config_files", {}
                )
    ...
    def load_configs_from_joplin_note(
        ...
        try:
            ...
            # 使用解析方法获取配置字典
            configs_dict, update_records = self.parse_config_from_markdown_table(
                note_content
            )

            # 关键修复：加载本地配置，用于补充缺失的配置信息
    ...
    def save_configs_to_local_smart(self, configs: Dict[str, Any]) -> None:
            ...
            if not config_file.exists():
                ...
                try:
                    ...
                    if existing_time != new_time:
                        ...
                        # 比较其他关键字段
                        existing_name = existing_config.get("system", {}).get(
                            "device_name", ""
                        )
                        ...
                        if existing_name != new_name:
                            should_save = True
                            save_reason = (
                                f"设备名称不同 ({existing_name} -> {new_name})"
                            )
                except Exception as e:
                    should_save = True
            ...
            if should_save:
                try:
                    ...
                    if config_file.exists():
                        saved_count += 1
                        log.info(
                            f"保存配置: {device_id} ({save_reason}) -> {config_file}"
                        )
                    else:
        ...
        # 统计报告
        log.info(
            f"配置保存统计: 保存{saved_count}个, 跳过{skipped_count}个, 无变化{updated_count}个"
        )
        # 清理过时的配置文件
        self._cleanup_old_configs(configs)

    def fix_config_with_local_data(
        self, parsed_configs: Dict[str, Any]
    ) -> Dict[str, Any]:
        """用本地配置修复解析的配置"""
        ...
        for device_id, config in fixed_configs.items():
            ...
            if needs_fix and device_id in local_configs:
                ...
                # 修复system字段
                if (
                    config["system"].get("host_user") == "N/A"
                    and "host_user" in local_config["system"]
                ):
                ...
                for key in [
                    ...
                    if (
                        config["system"]["system"].get(key) == "N/A"
                        and key in local_config["system"]["system"]
                    ):
                        config["system"]["system"][key] = local_config["system"][
                            "system"
                        ][key]
                # 修复python字段
                for key in [
                    ...
                    if (
                        config["python"].get(key) == "N/A"
                        and key in local_config["python"]
                    ):
                ...
                # 修复project
                if (
                    config["project"].get("project_path") == "N/A"
                    and "project_path" in local_config["project"]
                ):
                    config["project"]["project_path"] = local_config["project"][
                        "project_path"
                    ]
        ...
        for item in project_items:
            ...
            for did in device_ids:
                ...
                if item == "项目路径":
                    ...
                    value = len(
                        [k for k, v in config_files.items() if v != "Not found"]
                    )
                else:
                    value = "N/A"
                row.append(str(value))
                ...
                try:
                    if "T" in collection_time:
                        dt = datetime.fromisoformat(
                            collection_time.replace("Z", "+00:00")
                        )
    ...
    def generate_update_history(self, all_records: Dict[str, Any]) -> str:
        ...
        changes_count = sum(
            ...
            if u.get("has_changes", False) and u.get("summary", "无变化") != "无变化"
        )
        md_lines.append(
    ...
    def update_joplin_note(
        ...
        try:
            ...
            if existing_notes and len(existing_notes) > 0:
                ...
                # 更新标题显示更新时间
                new_title = (
                    f"{note_title} (更新于{datetime.now().strftime('%Y-%m-%d %H:%M')})"
                )
                ...
                # 创建新笔记
                note_id = createnote(
                    note_title, markdown_content, parent_id=notebook_id
                )
